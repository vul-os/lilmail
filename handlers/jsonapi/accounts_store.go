...
func (s *accountsStore) list(owner string) ([]connectedAccount, error) {
	...
	for k, b := range m {
		// The stored key is "<owner>|<email>"; guard against an owner name that
		// itself contains "|" fooling the prefix match by re-deriving the owner.
		if pipe := strings.Index(k, "|"); pipe < 0 || k[:pipe] != owner {
			continue
		}
